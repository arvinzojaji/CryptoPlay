$(document).ready(function () {
  ...
  if (loggedIn != null) {
    $("#loginPageBtn").html(
      "<a href='#' class='nav-link' id='signOut'>Sign Out of " +
        loggedIn +
        "</a>"
    );
      ...
      if (pwd == cfmPwd) {
        ...
        $.ajax({
          ...
          $.ajax({
            ...
            database
              .ref("/" + username + "/countryFlag")
              .set(flagResponse.location.country_flag_emoji);
  ...
  var top20QueryURL =
    "https://min-api.cryptocompare.com/data/top/totalvol?limit=20&tsym=USD&extraParams=CryptoPlay";
    ...
    if (loggedIn != null) {
      ...
      database
        ...
        .then(function (userSnap) {
              ...
              .then(function (userSnap) {
                ...
                if (userSnap.val().portfolio) {
                  for (var m = 0; m <= portfolioKeyArray.length - 1; m++) {
                    ...
                    var amountOwned =
                      userSnap.val().portfolio[purchasedObj].amountOwned;
                    console.log(amountOwned);
                    var paidPerCoin = userSnap
                      .val()
                      .portfolio[purchasedObj].avgPaidPerCoin.toFixed(2);

                    portfolioValue +=
                      userSnap.val().portfolio[purchasedObj].amountOwned *
                      parseFloat(
                        portfolioResponse.RAW[
                          Object.keys(portfolioResponse.RAW)[m]
                        ].USD.PRICE
                      );
                    ...
                    currentPortfolioPrice.append(
                      portfolioResponse.DISPLAY[
                        Object.keys(portfolioResponse.DISPLAY)[m]
                      ].USD.PRICE
                    );
                    trow
                      .append(coinName)
                  ...
                  var noCoin = $("<td>")
                    .attr("colspan", 4)
                    .html(
                      "<center>No coins in your portfolio. Go to <a href='index.html'>home page</a> to add coins</center>"
                    );
                  trow.html(noCoin);
                  $("#portfolio").html(trow);
                }
          ...
          $("#userBalance").html(
            "Available Funds: " + "&#8353;" + userSnap.val().balance.toFixed(2)
          );
        });
    } else {
      $("#userBalance").append(
        "<a href='signup.html' id='loginReg'>Login/Register</a>"
      );
      loggedIn = undefined;
      window.location.replace("signup.html");
    }
  ...
  if (
    !document.URL.includes("portfolio.html") &&
    !document.URL.includes("signup.html")
  ) {
    $("#loadingIcon").show();
    $.ajax({
      // get top 20 coins
      ...
      priceParams = Object.keys(topCoinsResponse.Data)
        .map((n) => topCoinsResponse.Data[n].CoinInfo.Internal)
        .join();
      ...
      $.ajax({
        ...
        for (var i = 0; i <= Object.keys(pricesObj).length - 1; i++) {
          ...
          var row = $("<tr>")
            .addClass("coinRow clickable")
            .attr("id", coinName)
            .attr("chart-loaded", "false")
            .attr("index", i)
            .attr(
              "currentPrice",
              pricesResponse.RAW[Object.keys(pricesResponse.RAW)[i]].USD.PRICE
            );

          // adding accordion functionality
          row
            .attr("data-toggle", "collapse")
            .attr("data-target", "#" + coinName + "-info")
            .attr("aria-expanded", "false")
            .attr("aria-controls", coinName + "-info")
            .attr("role", "button");

          var icon =
            "http://www.cryptocompare.com" + coinObjTopCoins.CoinInfo.ImageUrl;
          ...
          var colCoinSymbol = $("<td>")
            .addClass("coinSymbol")
            .attr("index", i)
            .html("<img src='" + icon + '\' width="35">');

          // Coin names under 'Name' column
          var colCoinName = $("<td>")
            .addClass("coinName")
            .attr("index", i)
            .text(coinObjTopCoins.CoinInfo.FullName);

          // Market Cap under 'Market Cap' column
          var colMarketCap = $("<td>")
            .addClass("marketCap")
            .attr("index", i)
            .text(coinObjPrices.USD.MKTCAP);

          // Price value under 'Price' column
          var colPrice = $("<td>")
            .addClass("price")
            .attr("index", i)
            .attr("emboldened", "false")
            .text(coinObjPrices.USD.PRICE);

          // Supply amount under 'Available Supply' column
          var colSupply = $("<td>")
            .addClass("availableSupply")
            .attr("index", i)
            .text(Math.round(coinObjTopCoins.ConversionInfo.Supply));
          ...
          var moreInfoRow = $("<tr>")
            .addClass("moreInfoRow collapse")
            .attr("id", coinName + "-info")
            .attr("aria-labelledby", coinName)
            .attr("data-parent", "#topCoinsTable");
          var moreInfoTd = $("<td>")
            .attr("colspan", "7")
            .addClass("moreInfoContainer");
          var buyAndSell = $("<form>")
            .attr("forindex", i)
            .addClass("buyAndSell")
            .css("float", "right");
          var howMuch = $("<input>")
            .addClass("howMuch")
            .attr("type", "text")
            .attr("placeholder", "Enter amount to Buy or Sell")
            .attr("index", i);
          var buyBtn = $("<button>")
            .addClass("btn btn-success buySellBtn buy")
            .text("Buy")
            .attr("index", i);
          var sellBtn = $("<button>")
            .addClass("btn btn-danger buySellBtn sell")
            .text("Sell")
            .attr("index", i);
          var totalDisplayP = $("<p>")
            .addClass("totalDisplay lead")
            .attr("index", i);
          ...
          row
            .append(colNum)
            .append(colCoinSymbol)
            .append(colCoinName)
            .append(colMarketCap)
            .append(colPrice)
            .append(colSupply)
            .append(col24hrChange);
          $("#topcoins").append(row);
          ...
          row.on("click", function () {
                ...
                $.ajax({
                  ...
                  for (var j = 0; j <= responseArray.length - 1; j++) {
                    chartData.labels.push(
                      moment.unix(responseArray[j].time).format("MM/DD/YY")
                    );
            ...
            function updatePrices(callback) {
              ...
              $.ajax({
                ...
                for (
                  var k = 0;
                  k <= Object.keys(innerPricesObj).length - 1;
                  k++
                ) {
                  var innerCoinObjPrices =
                    innerPricesObj[Object.keys(innerPricesObj)[k]];
    ...
    database
      ...
      .then(function (userbaseSnap) {
        ...
        for (var o = 0; o <= userArray.length - 1; o++) {
          if (
            moment
              .duration(now - userbaseSnap.val()[userArray[o]].lastOnline)
              .asDays() < 5
          ) {
            accountListPort.push([
              userArray[o],
              userbaseSnap.val()[userArray[o]],
            ]);
            if (indexPort <= 9) {
              var trowPort = $("<tr>")
                .addClass("leaderboardRow portRow")
                .attr("index", indexPort);
              var usernameLBPort = $("<td>")
                .addClass("portRowUser")
                .attr("index", indexPort);
              var valueLBPort = $("<td>")
                .addClass("portRowValue")
                .attr("index", indexPort);
          ...
          if (userbaseSnap.val()[userArray[o]].earnings > 0 && indexEarn <= 9) {
            accountListEarn.push([
              userArray[o],
              userbaseSnap.val()[userArray[o]],
            ]);
            if (indexEarn <= 9) {
              var trowEarn = $("<tr>")
                .addClass("leaderboardRow earnRow")
                .attr("index", indexPort);
              var usernameLBEarn = $("<td>")
                .addClass("earnRowUser")
                .attr("index", indexEarn);
              var valueLBEarn = $("<td>")
                .addClass("earnRowValue")
                .attr("index", indexEarn);
              var daysActive = $("<td>")
                .addClass("earnRowDays")
                .attr("index", indexEarn);
              trowEarn
                .append(usernameLBEarn)
                .append(valueLBEarn)
                .append(daysActive);
        ...
        accountListEarn.sort(function (obj1, obj2) {
          ...
          return (
            obj2[1].earnings / (now - obj2[1].accountBday) -
            obj1[1].earnings / (now - obj1[1].accountBday)
          );
        });
        ...
        for (
          var p = 0;
          p <= 9 &&
          (p <= accountListPort.length - 1 || p <= accountListEarn.length - 1);
          p++
        ) {
          if (accountListPort[p]) {
            console.log(accountListPort[p][0]);
            $(".portRowUser[index='" + p + "']").text(
              accountListPort[p][1].countryFlag + " " + accountListPort[p][0]
            );
            $(".portRowValue[index='" + p + "']").text(
              accountListPort[p][1].portfolioValue.toFixed(2)
            );
          }
          if (accountListEarn[p]) {
            console.log(accountListEarn[p][0]);
            $(".earnRowUser[index='" + p + "']").text(
              accountListEarn[p][1].countryFlag + " " + accountListEarn[p][0]
            );
            $(".earnRowValue[index='" + p + "']").text(
              accountListEarn[p][1].earnings.toFixed(2)
            );
            console.log(
              moment.duration(now - accountListEarn[p][1].accountBday).asDays()
            );
            $(".earnRowDays[index='" + p + "']").text(
              Math.ceil(
                moment
                  .duration(
                    now - accountListEarn[p][1].accountBday,
                    "milliseconds"
                  )
                  .asDays()
              )
  ...
  function buyNSell() {
    ...
    $(".buySellBtn")
      ...
      .click(function () {
        ...
        var selectedPrice = parseFloat(
          $(".coinRow[index='" + thisIndex + "']").attr("currentPrice")
        );
        var selectedCoin = $(".coinRow[index='" + thisIndex + "']").attr("id");

        if (loggedIn) {
          if (
            /^[0-9]*\.?[0-9]+$/.test(howMuchInput.val()) &&
            parseInt(howMuchInput.val()) > 0
          ) {
            ...
            var total =
              parseFloat(howMuchInput.val()) * parseFloat(selectedPrice);
            ...
            if ($(this).hasClass("buy")) {
                ...
                if (total > snap.val().balance) {
                  ...
                  if (!snap.val().portfolio[selectedCoin]) {
                    ...
                    database
                      ...
                      .set(parseFloat(howMuchInput.val()));
                    ...
                    database
                      ...
                      .set(
                        parseFloat(howMuchInput.val()) +
                          snap.val().portfolio[selectedCoin].amountOwned
                      );
                    // calculate average paid per coin
                    database
                      .ref(
                      ...
                      .set(
                        (snap.val().portfolio[selectedCoin].amountOwned *
                          snap.val().portfolio[selectedCoin].avgPaidPerCoin +
                          total) /
                          (snap.val().portfolio[selectedCoin].amountOwned +
                            parseFloat(howMuchInput.val()))
                      );
              ...
              userRef.once("value").then(function (snap) {
                ...
                if (!snap.val().portfolio[selectedCoin]) {
                  howMuchInput.attr(
                    "placeholder",
                    "You don't own any " + selectedCoin
                  );
                  howMuchInput.val("");
                } else {
                  ...
                  if (
                    ...
                    var earning =
                      ...
                      snap.val().portfolio[selectedCoin].avgPaidPerCoin *
                        snap.val().portfolio[selectedCoin].amountOwned;
                    database
                      .ref("/" + loggedIn + "/earnings")
                      .set(snap.val().earnings + earning);
                    ...
                    if (
                      ...
                      if (Object.keys(snap.val().portfolio).length == 1) {
                        ...
                        database
                          .ref("/" + loggedIn + "/portfolio/" + selectedCoin)
                          .remove();
                      }
                    } else {
                      console.log(
  ...
  userWalletRef.on("value", function (walletSnap) {
    $("#userBalance").html(
      "Available Funds: " + "&#8353;" + walletSnap.val().toFixed(2)
    );
  });
});
